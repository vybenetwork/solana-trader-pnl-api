...
import { VYBE_API_BASE, VYBE_TIMEOUT_MS, VYBE_MAX_RETRIES, VYBE_RETRY_DELAY_MS, } from '../config.js';
/**
 ...
 */
export function toHumanReadableError(err) {
    if (axios.isAxiosError(err)) {
        ...
        if (msg && typeof msg === 'string')
            return msg;
        if (status)
            return `API returned ${status} for ${endpoint}.`;
    }
