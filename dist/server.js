...
app.get('/api/wallets/top-traders', async (req, res) => {
    try {
        ...
        const pageRaw = queryOne(req, 'page');
        const page = pageRaw != null && !Number.isNaN(Number(pageRaw)) && Number(pageRaw) >= 0
            ? Number(pageRaw)
            : undefined;
...
app.get('/api/wallets/:ownerAddress/pnl', async (req, res) => {
    try {
...
app.get('/api/wallets/:ownerAddress/pnl-ts', async (req, res) => {
    try {
        const ownerAddress = param(req, 'ownerAddress').trim();
        if (!ownerAddress)
            return res.status(400).json({ error: 'Owner address required' });
        ...
        const timeStartRaw = queryOne(req, 'timeStart');
        const timeEndRaw = queryOne(req, 'timeEnd');
        const timeStartNum = timeStartRaw != null && timeStartRaw !== '' && !Number.isNaN(Number(timeStartRaw))
            ? Number(timeStartRaw)
            : undefined;
        const timeEndNum = timeEndRaw != null && timeEndRaw !== '' && !Number.isNaN(Number(timeEndRaw))
            ? Number(timeEndRaw)
            : undefined;
        const timeStart = timeStartNum !== undefined && Number.isFinite(timeStartNum) && timeStartNum >= 0
            ? Math.floor(timeStartNum)
            : undefined;
        const timeEnd = timeEndNum !== undefined && Number.isFinite(timeEndNum) && timeEndNum >= 0
...
app.get('/api/tokens/:mint/top-pnl-traders', async (req, res) => {
    try {
        const mint = param(req, 'mint').trim();
        if (!mint)
            return res.status(400).json({ error: 'Mint address required' });
